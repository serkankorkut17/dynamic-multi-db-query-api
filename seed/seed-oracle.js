...
async function dropSchema(conn){
  await conn.execute(`
    BEGIN
      EXECUTE IMMEDIATE 'DROP TABLE course_prerequisites'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF;
    END;`);
  await conn.execute(`
    BEGIN
...
async function createSchema(conn){
  ...
  await conn.execute(`CREATE TABLE course_prerequisites (
    id NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    course_id NUMBER NOT NULL REFERENCES courses(id) ON DELETE CASCADE,
    prerequisite_course_id NUMBER NOT NULL REFERENCES courses(id) ON DELETE CASCADE,
  ...
  for (const schoolId of schoolIds){
    ...
    for (let i=0; i<thisSchoolCourses.length; i++){
      ...
      for (const prereqId of selected){
        try {
          await conn.execute(`INSERT INTO course_prerequisites(course_id, prerequisite_course_id) VALUES (:c,:p)`, { c: courseId, p: prereqId });
        } catch (e) { /* ignore dup */ }
      }
    }
